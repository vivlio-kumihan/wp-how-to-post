...
import { DetailError, assertVivliostyleConfigSchema, cwd, debug, isUrlString, log, logWarn, openEpubToTmpDirectory, readJSON, statFileSync, touchTmpFile, upath, } from '../util.js';
...
export async function collectVivliostyleConfig(cliFlags) {
    const load = async (configPath) => {
        ...
        try {
            assertVivliostyleConfigSchema(config, typeof jsonRaw === 'string' ? { json: jsonRaw } : { indent: 2 });
        }
        catch (error) {
            const thrownError = error;
            throw new DetailError(`Validation of vivliostyle.config failed. Please check the schema: ${configPath}`, typeof thrownError === 'string'
                ? thrownError
...
async function composeProjectConfig(otherConfig, cliFlags, config, context) {
    ...
    const isContentsEntry = (entry) => entry.rel === 'contents' && !('path' in entry);
